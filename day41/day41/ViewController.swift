...
class ViewController: UIViewController {
        ...
        if currentSolution.contains(letterCharacter){
            ...
            if !progress.contains("?"){
                if score + 1 == cluesAnswers.count{
                    let vc = UIAlertController(title: "Congratulations!!! You win", message: "Want to go to next level?", preferredStyle: .alert)
                    vc.addAction(UIAlertAction(title: "OK", style: .default, handler: levelUp))
    ...
    func loadData(){
        cluesAnswers.removeAll(keepingCapacity: true)
    ...
    func getData(){
        ...
        if let levelFileURL = Bundle.main.url(forResource: "level\(level)", withExtension: "txt"){
            if let levelContents = try? String(contentsOf: levelFileURL){
                cluesAnswers = levelContents.components(separatedBy: "\n")
                
                cluesAnswers.shuffle()
