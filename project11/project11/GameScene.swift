...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        balls += ["ballBlue", "ballCyan", "ballYellow", "ballRed", "ballGreen", "ballGrey", "ballPurple"]
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if objects.contains(editLabel){
            ...
            if editingMode{
                ...
                if ballsCount < 10{
                    let sizeOfBalls = balls.count
                    let ball = SKSpriteNode(imageNamed: balls[Int.random(in: 0..<sizeOfBalls)])
                    ball.physicsBody = SKPhysicsBody(circleOfRadius: ball.size.width / 2.0)
                    ball.physicsBody!.contactTestBitMask = ball.physicsBody!.collisionBitMask
                    ball.physicsBody?.restitution = 0.4
                    
                    ball.position = CGPoint(x: location.x, y: self.size.height - 68)
                    ball.name = "ball"
                    addChild(ball)
                    ballsCount += 1
                }
    ...
    func collisionBettween(ball: SKNode, object: SKNode){
        if object.name == "good"{
            ...
            if boxes == 0{
                ballsCount -= 1
                if let magicParticle = SKEmitterNode(fileNamed: "MyParticle"){
